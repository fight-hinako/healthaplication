...
>
    ...
    <span id="task-countdown-display" class="text-2xl text-green-600/75 font-semibold tabular-nums shrink-0 px-5 pb-3">00:00</span>
    ...
    <div class="flex flex-col divide-y divide-[#f0f1f3] w-full flex-1 min-h-0 overflow-y-auto">
        @foreach ($tasks as $task)
            ...
            >
                ...
                <span class="task-duration text-lg tabular-nums px-2 py-1 rounded-xl transition-colors group-[.is-checked]:text-[#8b919a] group-[.is-checked]:bg-[#eceef1]">{{ $task['duration'] }}</span>
            </button>
        @endforeach
    </div>
</div>
